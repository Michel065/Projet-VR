...
public class Perso1PathMoveOnDialogueDone : MonoBehaviour
{
    [Header("Dialogue")]
    public NPCDialogue npcDialogue; // glisse le Dialogue Manager ici

    [Header("Move")]
    public Transform objectToMove; // glisse le NPC ici
    ...
    private void Update()
    {
        if (!started && npcDialogue != null && npcDialogue.dialogueDone)
        {
            started = true;
            currentPointIndex = 0;
        ...
        if (target == null) { currentPointIndex++; return; }
        ...
        if (rotateTowardsTargetWhileMoving)
        {
            ...
            if (dir.sqrMagnitude > 0.001f)
            {
                ...
                objectToMove.rotation = Quaternion.Slerp(
                    objectToMove.rotation, lookRot, rotationSpeed * Time.deltaTime);
            }
        }

        objectToMove.position = Vector3.MoveTowards(
            currentPosition, targetPosition, speed * Time.deltaTime);
